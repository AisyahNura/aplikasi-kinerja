{{-- Halaman daftar staff untuk KASI - menampilkan staff yang berada di bawah KASI --}}
@extends('layouts.kasi')

{{-- Section content untuk konten utama halaman --}}
@section('content')
...
<div class="p-3 sm:p-6">
    <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-2 sm:ml-4">
                        ...
                        {{-- Menampilkan jumlah total staff dari controller --}}
                        <p class="text-lg sm:text-2xl font-semibold text-gray-900">{{ $totalStaff }}</p>
                    </div>
                </div>
            </div>

            {{-- Kartu staff aktif dengan icon check --}}
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-6">
                <div class="flex items-center">
                    {{-- Icon check dengan background biru --}}
                    ...
                    <div class="ml-2 sm:ml-4">
                        ...
                        {{-- Menampilkan jumlah staff aktif dari controller --}}
                        <p class="text-lg sm:text-2xl font-semibold text-gray-900">{{ $staffAktif }}</p>
                    </div>
                </div>
            </div>

            {{-- Kartu staff nonaktif dengan icon warning --}}
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-2 sm:ml-4">
                        ...
                        {{-- Menampilkan jumlah staff nonaktif dari controller --}}
        ...
        {{-- Container daftar staff dengan card layout --}}
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            {{-- Content area untuk daftar staff --}}
            <div class="p-4 sm:p-6">
                {{-- Cek apakah ada staff yang ditampilkan --}}
                @if($staffList->count() > 0)
                    {{-- Grid layout responsive untuk card staff --}}
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                        {{-- Loop untuk setiap staff dalam daftar --}}
                        @foreach($staffList as $staff)
                            {{-- Card individual staff dengan border dan background --}}
                            <div class="bg-gray-50 rounded-lg p-3 sm:p-4 border border-gray-200">
                                <div class="flex items-center">
                                    ...
                                    <div class="w-8 h-8 sm:w-10 sm:h-10 bg-green-600 rounded-full flex items-center justify-center mr-3 sm:mr-4">
                                        {{-- Ambil huruf pertama nama dan jadikan uppercase --}}
                                        <span class="text-white font-semibold text-xs sm:text-sm">{{ strtoupper(substr($staff->name, 0, 1)) }}</span>
                                    </div>
                                    
                                    {{-- Informasi detail staff --}}
                                    <div class="flex-1 min-w-0">
                                        {{-- Nama staff dengan truncate jika terlalu panjang --}}
